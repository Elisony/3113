<template>
	<view class="content">
		...
		<view class="option-box">
			...
			<navigator url="/pages/myTask/myTask" hover-class="navigator-hover" class="option-right">
				已取消
			</navigator>
		</view>
		<view style="width: 100%;" v-for="(item, index) in taskList" :key="index">
			<view class="order-list">
				...
				<navigator :url="`/pages/orderFinish/orderFinish?id=${item.order_id}`" hover-class="navigator-hover" class="button-box">
					查看订单详情
				</navigator>
			</view>
		</view>
			...
			getTask(){
				...
				if (userData) {
					uni.request({
						...
						data: {
							interface: "order_getCompleteOrderList",
							data: {
